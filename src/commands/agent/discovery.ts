...
export function parseSkillFrontmatter(content: string): {
  ...
  // Simple frontmatter parser (avoid gray-matter dependency)
...
/** Discover skills from a directory (looks for SKILL.md files) */
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (parsed) {
          ...
          // SKILL.md exists but no valid frontmatter — use directory name
          assets.push({
            name: entry.name,
            description: '',
            sourcePath: skillDir,
